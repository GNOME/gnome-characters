...
const CharacterListRow = GObject.registerClass({
    ...
    draw(cr, x, y, width, height) {
        ...
        const cellSize = getCellSize(this._fontDescription);
        for (const i in this._characters) {
            ...
            if (Gc.character_is_invisible(this._characters[i])) {
                ...
                if (layout.get_unknown_glyphs_count () == 0) {
                    const layoutBaseline = layout.get_baseline();
                    const [logicalRect, inkRect] = layout.get_extents();
    ...
    _computeBoundingBox(cr, cellRect, uc) {
        ...
        if (layout.get_unknown_glyphs_count() == 0) {
            ...
            if (this._baseGlyphRect == null) {
                layout.set_text('A', -1);
                const [baseLogicalRect, baseInkRect] = layout.get_extents();
            ...
            const characterWidth = Gc.character_width (uc);
            if (characterWidth > 1)
    ...
    _drawBoundingBox(cr, cellRect, uc) {
        ...
        layout.set_font_description(this._fontDescription);
        layout.set_text(uc, -1);
        const shapeRect = this._computeBoundingBox(cr, cellRect, uc);
    ...
    _drawCharacterName(cr, cellRect, uc) {
        ...
        const text = name == null ? _('Unassigned') : Util.capitalize(name);
...
const CharacterListWidget = GObject.registerClass({
    ...
    vfunc_drag_begin(context) {
        ...
        this._dragSurface = new Cairo.ImageSurface(Cairo.Format.ARGB32,
                                                   cellSize,
                                                   cellSize);
        const cr = new Cairo.Context(this._dragSurface);
        cr.setSourceRGBA(1.0, 1.0, 1.0, 1.0);
        cr.paint();
        cr.setSourceRGBA(0.0, 0.0, 0.0, 1.0);
        const row = this._createCharacterListRow([this._character]);
    ...
    vfunc_button_press_event(event) {
        ...
        const index = y * this._cellsPerRow + x;
    ...
    vfunc_get_preferred_height_for_width(width) {
        const height = Math.max(this._rows.length, this._numRows) *
    ...
    vfunc_get_preferred_width_for_height(height) {
        const cellSize = getCellSize(this._fontDescription);
        const minWidth = NUM_COLUMNS * cellSize;
        const natWidth = Math.max(this._cellsPerRow, NUM_COLUMNS) * cellSize;
    ...
    vfunc_size_allocate(allocation) {
        ...
        const cellSize = getCellSize(this._fontDescription);
        const cellsPerRow = Math.floor(allocation.width / cellSize);
    ...
    _createCharacterListRow(characters) {
        ...
        const row = new CharacterListRow({
    ...
    setCharacters(characters) {
        ...
        for (; stop <= characters.length; stop++) {
            if (stop % this._cellsPerRow == 0) {
                const rowCharacters = characters.slice(start, stop);
                const row = this._createCharacterListRow(rowCharacters);
                this._rows.push(row);
                start = stop;
            }
        }
    ...
    vfunc_draw(cr) {
        // Clear the canvas.
        const context = this.get_style_context();
        const fg = context.get_color(Gtk.StateFlags.NORMAL);
        const bg = context.get_background_color(Gtk.StateFlags.NORMAL);
        ...
        const [x1, y1, x2, y2] = cr.clipExtents();
        const cellSize = getCellSize(this._fontDescription);
        const start = Math.max(0, Math.floor(y1 / cellSize));
        const end = Math.min(this._rows.length, Math.ceil(y2 / cellSize));
...
var FontFilter = GObject.registerClass({
    ...
    set font(v) {
        const fontDescription = Pango.FontDescription.from_string(v);
    ...
    apply(widget, characters) {
        ...
        if (this._filterFontDescription) {
            ...
            const filterFont = context.load_font(this._filterFontDescription);
            const filteredCharacters = [];
            for (let index = 0; index < characters.length; index++) {
                const uc = characters[index];
...
var CharacterListView = GObject.registerClass({
    ...
    _init(params) {
        ...
        const context = scroll.get_style_context();
    ...
    get initialSearchCount() {
        ...
        const cellSize = getCellSize(this._fontFilter.fontDescription);
        const cellsPerRow = Math.floor(allocation.width / cellSize);
        // Ensure the rows cause a scroll
        const heightInRows = Math.ceil((allocation.height + 1) / cellSize);
    ...
    searchByCategory(category) {
        ...
        const criteria = Gc.SearchCriteria.new_category(category.category);
