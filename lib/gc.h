...
G_DECLARE_FINAL_TYPE (GcSearchContext, gc_search_context, GC, SEARCH_CONTEXT,
                      GObject)
...
GType gc_search_criteria_get_type(void);

GcSearchCriteria * gc_search_criteria_new_category(GcCategory category);

GcSearchCriteria * gc_search_criteria_new_keywords(const gchar *const *keywords);

GcSearchCriteria * gc_search_criteria_new_scripts(const GUnicodeScript *scripts,
                                                  size_t                n_scripts);

GcSearchCriteria * gc_search_criteria_new_related(const gchar *character);

GcSearchContext * gc_search_context_new(GcSearchCriteria *criteria,
                                        GcSearchFlag      flags);
void gc_search_context_search(GcSearchContext    *context,
                              gint                max_matches,
                              GCancellable       *cancellable,
                              GAsyncReadyCallback callback,
                              gpointer            user_data);

GcSearchResult * gc_search_context_search_finish(GcSearchContext *context,
                                                 GAsyncResult    *result,
                                                 GError         **error);

gboolean gc_search_context_is_finished(GcSearchContext *context);

gboolean gc_character_is_composite(const gunichar *chars,
                                   int             n_chars);

gchar * gc_character_name(const gunichar *chars,
                          int             n_chars);
gboolean gc_character_is_invisible(const gunichar *chars,
                                   int             n_chars);

gchar * gc_get_current_language(void);

GUnicodeScript * gc_get_scripts_for_language(const gchar *language,
                                             size_t      *n_scripts);
